...
class Assembler:
    ...
    def assemble_image(folder_path, tile_grid, output_img_name):
        ...
        for i in range(0, edge_length_x):
            ...
            for j in range(0, edge_length_y):
                ...
                tmp_img = PIL.Image.open(
                    folder_path + str(i) + '.' + str(j) + '.png')
    ...
    def crop_image(map_box, img_path, output_path, zoom):
        ...
        tl_coord = Coord(mercantile.ul(tl_tile).lng,
                         mercantile.ul(tl_tile).lat)
        br_coord = Coord(mercantile.ul(br_tile).lng,
                         mercantile.ul(br_tile).lat)
        ...
        pixel_diff_left = round(
            (diff_left_degree / total_x_degree) * img.width)
        pixel_diff_right = round(
            (diff_right_degree / total_x_degree) * img.width)
        pixel_diff_top = round((diff_top_degree / total_y_degree) * img.height)
        pixel_diff_bottom = round(
            (diff_bottom_degree / total_y_degree) * img.height)

        # Crop Image. Save as output_path
        crop_payload = (0 + pixel_diff_left, 0 + pixel_diff_top,
    ...
    def add_pins_to_map(context, input_path: str, output_path: str) -> None:
       for p in context['pins']:
            curr_pin = Pin(icon=p['icon'], location=p['location'], digital_height=p['digital_height'], digital_width=p['digital_width'], color=p['color'])
            logging.info("adding pin: " + str(curr_pin))
            Assembler.add_pin(context, input_path=input_path, output_path=output_path, pin=curr_pin) 
    ...
    def add_transparency(img_path: str, img_output_path:str):
        ...
        alpha = arr[:, :, 3]
        n = len(alpha)
        alpha[:] = np.interp(np.arange(n), [0, 0.75*n, 0.95*n, n], [255, 255, 0, 0])[:,np.newaxis]
    ...
    def add_text(img_path:str, out_path:str, msg: list):
        ...
        draw.text(((width-w)/2,(height-h)*.94), msg[0], fill="black", font=myFont)    

        myFont = ImageFont.truetype('src/fonts/texgyreadventor-regular.otf', 150)
        #Center the text 
        w, h = draw.textsize(msg[1], font=myFont)
        draw.text(((width-w)/2,(height-h)*.97), msg[1], fill="black", font=myFont)    
